$(document).ready(function(){
  ...
  $('#file').on('click','.deleteMediaButton',function(){
    ...
    {
      $.ajax({
        url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.DeleteSingleMedia',
        type: 'POST',
        data: {
          id: idMedia
        },
  ...
  $('#list_options').on('change',function(){
    var type = $(this).val();
    $.ajax({
      url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.SetRelationType',
      type: 'POST',
      data: {
        type: type,
      },
  ...
  $('.btn-dam').on('click',function(){
    ...
    $.ajax({
      url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.CreateMediaFromDAM',
      type: 'POST',
      data: {
        media: media,
        id: id
  ...
  $('#showElements').on('click','.js-actionStru',function(){
    ...
    if(type == 'saveAll')
    {
      ...
      if(option == 0 || option == 2)
      {
        var importAll = true;
        $.ajax({
          url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.CreateMedia',
          type: 'POST',
          data: {
            type: type,
            stru: stru,
    ...
    else if(type == 'deleteAll')
    {
      ...
      {
        $.ajax({
          url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.DeleteMedia',
          type: 'POST',
          data: {
            id: id,
            option: option
    ...
    else if(type == 'saveCheckedMedia')
    {
      ...
      {
        $.ajax({
          url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.CreateMediaFromList',
          type: 'POST',
          data: {
            stru: stru,
            key: key,
  ...
  $('input[name="linkedStru"]').on('change',function(){
    ...
    if(value)
    {
      ...
      // $.ajax({
      //   url: Glizy.ajaxUrl + '&controllerName=metafad.teca.MAG.controllers.ajax.CheckMag',
      //   type: 'POST',
      //   data: {
      //     id: value.id
      //   },
