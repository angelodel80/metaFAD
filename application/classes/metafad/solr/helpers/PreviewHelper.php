<?php
class metafad_solr_helpers_PreviewHelper extends GlizyObject
{
            ...
            if ($data->$key) {
                ...
                if (is_string($d) && $d) {
                    if($key == 'FTA-image')
                    {
                        ...
                        $viewerHelper = __ObjectFactory::createObject('metafad.viewer.helpers.ViewerHelper');
                        $dam = $viewerHelper->initializeDam($instituteKey);
                        $json = json_decode($d);
                        $imageUrl = metafad_teca_DAM_Common::replaceUrl($dam->streamUrl($json->id, 'thumbnail'));
